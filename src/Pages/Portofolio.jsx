...
export default function FullWidthTabs() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const [projectRes, certRes] = await Promise.all([
        fetch("/data/projects.json"),
        fetch("/data/certification.json"),
      ]);

      if (!projectRes.ok) throw new Error("Failed to load projects from file");
      if (!certRes.ok) throw new Error("Failed to load certificates from file");

      const projectData = await projectRes.json();
      const certificateData = await certRes.json();
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 2xl:grid-cols-3 gap-5">
              {displayedProjects.map((project, index) => (
                <div
                  ...
                  data-aos={["fade-up-right", "fade-up", "fade-up-left"][index % 3]}
